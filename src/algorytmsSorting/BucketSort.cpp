...
static bool pushBack(Node*& head, Node*& tail, int value) {
    // tworzymy nowy węzeł
...
// sortuje pojedynczą listę za pomocą insertion sorta (każda lista reprezentuje jeden kubełek)
...
static void clearBuckets(Node** heads, int bucketCount) {
...
bool BucketSort::sort(Array& array) {
    const int size = array.getSize();    //  rozmiar tablicy


    // jeśli tablica jest pusta albo ma jeden element - posortowana
    if (size <= 1) {
        return true;
    }
    ...
    for (int i = 1; i < size; ++i) {
        ...
        if (array.setId(i) < minValue) {
            minValue = array.setId(i);
        }

        // jeśli aktualny element jest większy od maxValue - aktualizujemy maksimum
        if (array.setId(i) > maxValue) {
            maxValue = array.setId(i);
        }
    }
    ...
    // rozdzielamy elementy tablicy do kubełków
    for (int i = 0; i < size; ++i) {
        const int value = array.setId(i);   // pobieramy aktualną wartość z tablicy
    ...
    int index = 0;    // index - gdzie wpisujemy kolejne elementy do tablicy wynikowej


    // przepisujemy elementy z kubełków z powrotem do tablicy
    for (int i = 0; i < bucketCount; ++i) {
        Node* current = bucketHeads[i];        // zaczynamy od początku listy w danym kubełku

        while (current != nullptr) {
            array.setId(index) = current->value;    // wpisujemy wartość z listy do tablicy
            ++index;
